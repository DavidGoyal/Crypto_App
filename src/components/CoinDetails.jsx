  ...
  useEffect(() => {
    const fetchCoin = async () => {
      try {
        ...
        const { data: chartArr } = await axios.get(`${server}/coins/${params.id}/market_chart?vs_currency=${currency}&days=${days}`);
        setCoin(data);
        ...
        <>
          ...
          <HStack wrap={"wrap"} p={4}>
            {btns.map((btn, index) => (
              <Button
                key={index}
                onClick={() => {setDays(btn),setLoading(true)}}
                bgColor={days === btn ? "blackAlpha.800" : "blackAlpha.200"}
                color={days === btn ? "white" : "blackAlpha.800"}
              >
                {btn}
              </Button>
            ))}
